...
def bearing(coord1, coord2):
    ...
    ### returns bearing based on unit circle (90 deg is up, not East)
...
def beta_calc_pt(source, mwx, prf):
    ...
    bearing_drone_d = bearing(coord_s, coord_r) - 90 # bearing of drone from source in [deg] and convert to unit circle
...
def beta_calc_area(source, mwx, prf):
    ...
    ### set source variables
    ### for area source, lat/long should be the SW most corner of the rectangle
    Lat_s = source['LATITUDE_S']
    Long_s = source['LONGITUDE_S']
    coord_s = (Lat_s, Long_s)
    ...
    src_wind_ang = abs(orient_ang - wd_compare) # by compass
    ...
    h = haversine_r(coord_s, coord_r)  # haversine distance of drone from SW corner source in [km]
    bearing_drone_d = bearing(coord_s, coord_r) - 90 # bearing of drone from SW corner source in [deg] and convert to unit circle
    theta_d = abs(wd_d - bearing_drone_d) # absolute difference of wind direction from drone bearing
    theta_r = math.radians(theta_d) # convert to radians
    dx = round(h * math.cos(theta_r), 3) # x component distance relative to wind direction in [km]
    dx_r = dx - (x_dist_new/2) # move source coord from SW corner to middle of area (x-direction)
    
    dy = round(h * math.sin(theta_r), 3) * 1000 # y component distance relative to wind direction in [m]
    dy_r = dy - (y_dist_new/2) # move source coord from SW corner to middle of area (y-direction)
    ...
    sigma_y_o = (x_dist/4.3)*1000 # get initial sigma based on x length and convert t0 [m]
