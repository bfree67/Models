...
data_file_name = "fullset-48hrlag-8hr.xlsx"
...
drop_out = 0.15
n_batch = 72
newpath = 'E:\\\Main\PythonML\\'   #example output path
...
tot_err_summary = np.sum(tot_err, axis = 0)/(N_test)
tot_err_per = np.sum(tot_err)/(N_test*n_outputs)

end_train = time.clock() ###### stop training clock 
  
if (end_train-start_train > 60):
    print"\n Trained and tested in {0:.2f} minutes with {1} data points".format((end_train-start_train)/60., N)
else:
    print"\n Trained and tested in {0:.2f} seconds with {1} data points".format((end_train-start_train)/1.,N)
    #print" with", N, "data points"

print " Error = {0:.2f}%".format(tot_err_per * 100)

filename = 'FinErr_mlp_epoch'+ str(n_epochs) +'_batch' + str(n_batch) + '.xlsx'    #example output file
writer = ExcelWriter(filename)
pd.DataFrame(tot_err_summary).to_excel(writer,'FinalError')
writer.save()
